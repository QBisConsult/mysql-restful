	...
	if (utils.findin(accepted_list,table)){
		...
		function rapidfind(){
			if (table == 'rpdquery'){
				...
				//res.status(401);return res.send({error:'Query error/Primary key not found in GET:Query parameters'})
		...
		function rapidcreate(){
			...
			} else {res.status(401);return res.send({error:'Query error/No parameters found-POST:Body/required format JSON'})}
		};
		function rapidupdate(){
			if (req.body[primary_key]){
				rdb.update(req,table,primary_key,table_structure,dbcon,rediscon,rdbcallback)
			} else {res.status(401);return res.send({error:'Query error/Primary key not found in PUT:Body parameters'})}
		};
		function rapiddelete(){
			...
			} else {res.status(401);return res.send({error:'Query error/Primary key not found in DELETE:Query parameters'})}
		};
		switch (rmethod) {
